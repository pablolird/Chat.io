...
class Message(QWidget):
    def __init__(self, username, text: str, timestamp: str, is_sender: bool = False):
        ...
        if is_sender:
            ...
            self.bubble = QLabel(username+": "+text)
            layout.addWidget(self.timestamp)
            layout.addWidget(self.bubble)
            layout.addStretch()
            self.bubble.setStyleSheet("background-color: grey; border-radius: 10px; padding: 10px; color: white;")

        self.setLayout(layout)
        self.bubble.setWordWrap(True)
        self.bubble.setSizePolicy(QSizePolicy.Preferred, QSizePolicy.Preferred)
        self.bubble.setMaximumWidth(400)  # You can adjust or use a percentage
